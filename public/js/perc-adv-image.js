(function () {
  ...
  // AR
  function onImageResponse(event) {
    var detail = event.detail;
    switch (detail.type) {
      case 'pc-image-resp':
        console.log('pgImage pc-image-resp', detail.data);
        if (detail.data && detail.data.error) {
          $('.perc-alert').alert('Error!', 'Could not obtain photo').show();
        } else {
          // we got image
          $('[aria-label="Insert/edit image"]').find('.mce-textbox').first().val(detail.data.dataUri);
        }
        break;
      default:
        // unknown event was received
        break;
    }
  }
  ...
  tinymce.PluginManager.add('percAdvImage', function (editor) {
    ...
    function showDialog(imageList) {
      ...
      // AR
      function onPostRender(event, f) {
        var args = editor.windowManager.getParams(),
          service = args.service;
        $('[aria-label="Insert/edit image"]').find('.mce-btn.mce-open').click(function () {
          service.off(document, service.CUSTOM_EVENT_NAME, onImageResponse);
          service.on(document, service.CUSTOM_EVENT_NAME, onImageResponse);
      ...
      if (editor.settings.image_advtab) {
        ...
        win = editor.windowManager.open({
          ...
          onPostRender: onPostRender // AR
        }, { service: PercEditorService }); // AR
      } else {
